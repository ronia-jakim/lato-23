...
\begin{definition} \important{Rzędem funkcji} $f:M\to N$ $C^1$-różniczkowalnego (def. \ref{definicja:1.12}) w punkcie $p$ nazywamy rząd macierzy pierwszych pochodnych cząstkowych odwzorowania $\psi f\phi^{-1}$ w $\phi(p)$.
\end{definition}
...
\begin{definition}[dyfeomorfizm]\label{definicja:dyfeomorfizmy} Gładkie odwzorowanie $f:M\to N$ nazywamy \important{dyfeomorfizmem}, jeśli jest wzajemnie jednoznaczne (bijekcja) oraz odwzorowanie do niego odwrotne $f^{-1}$ jest gładkie.

  Dwie rozmaitości $M, N$ są \acc[b]{dyfeomorficzne}, jeśli istnieje między nimi dyfeomorfizm. Są one wtedy nierozróżnialne jako gładkie rozmaitości.
\end{definition}
...
\begin{definition}[odwzorowanie $C^k$-różniczkowalne]\label{definicja:1.12}
...
%  \node (a) at (26:2.5) {$\tilde{M}$};
  ...
  \draw (11, -4) ellipse (2.5 and 1.3);
  \draw (9.8, -3.85) arc (200:340:1.3 and 0.6);
  \draw (10, -4) arc (170:10:1 and 0.4);

  \filldraw[orange!40] (5.9, 0.8) arc (90:-120:0.6);
  \node at (6.5, 1.05) {$\color{orange}U\cap f^{-1}(V)$};
  \node at (5.1, 0.5) {$U$};
  
  \filldraw (6, 0.3) circle (2pt) node [below] {$p$};
  \filldraw (9.5, -3.5) circle (2pt);% node [above] {$f(p)$};
  \node at (9.6, -3) {$f(p)$};

  \path[->] (6.1, 0.3) edge [bend left=20] node [above] {$f$} (9.4, -3.5);

  \draw (5.9, 0.2) circle (0.6);
  \draw (9.4, -3.7) ellipse (0.4  and 0.6);

  \node at (9.6, -4.5) {$V$};

%    \draw[double distance=5mm] () arc (0:180:1);
%    \draw[double distance=5mm] () arc (180:360:1);

  \draw[<-] (7, -3)--(7, -5);
  \draw[->] (6, -4)--(8, -4);

  \draw (7.2, -3.8) ellipse (0.5 and 0.7);
  \filldraw (7.2, -3.8) circle (2pt) node [below] {$\psi(f(p))$};
  \node at (6.8, -3) {$\psi(V)$};
  \path[->] (9, -3.5) edge [bend right=20] node [above] {$\psi$} (7.7, -3.7);


  \filldraw[orange!40] (3,-2.9) arc (90:-120:0.6);

  \draw[<-] (3, -2.5)--(3, -4.5);
  \draw[->] (2, -3.5)--(4, -3.5);

  \draw (3, -3.5) circle (0.6);
  \filldraw (3.3, -3.3) circle (2pt) node [below] {$\phi(p)$};

  \path[->] (5.3, -0.1) edge [bend right=20] node [above] {$\phi$} (3.2, -2.9);
  \node at (4.2, -4.3) {$\color{orange}\phi[U\cap f^{-1}(V)]$};
  \node at (2.5, -2.7) {$\phi(U)$};

  \path[->] (3.4, -3.1) edge [bend left=20] node [above] {$\psi\circ f\circ\phi^{-1}$} (6.7, -3.7);
\end{tikzpicture}
}
\end{center}

Funkcję $\psi\circ f\circ\phi^{-1}$ jest nazywana \important{wyrażeniem $f$ w mapach} $(U,\phi)$ i $(V,\psi)$ lub też wyrażeniem $f$ w lokalnych współrzędnych zadanych przez te mapy.
...
\begin{definition}[rozmaitość z brzegiem]\label{definicja:1.19}
