...
Let $m=p^k+1$ such that $p^k>[L:K]$. It is clear that $p\nmid m$. Now, consider the polynomial
$$w_m(x)=x^m-1=(x-1)\underbrace{(x^{m-1}+x^{m-2}+...+x+1)}_{v_m(x)}$$
and we will show that $(x-a_i)|v_m$ for every $i=1,...,r$ implying that $v_m\in I(a_i/K)=(g)$ and so $g|v_m$ and $g|w_m=(x-1)v_m$.

The observation is quite simple: we took $m$ to be such that $m-1\geq [L:K]$. Thus, 
