...
\begin{definicja}[wielomian minimalny, stopień pierwiastka]
...
\begin{itemize}
    \item[\point] $f$ jest nazywany wielomianem \deff{minimalnym} $a$ nad $K$ (wyznaczony jednoznacznie)
    \item[\point] \acc{stopień $a$} nad $K$ jest definiowany jako $deg(f)$.
\end{itemize}
\end{definicja}
...
\begin{wniosek}[$K_{alg}$ - podciałem]
...
\begin{definicja}[(relatywne) algebraiczne domknięcie]{\color{back}spaaaać}
    \begin{enumerate}
        \item $K_{alg}(L)$ nazywamy \deff{algebraicznym domknięciem} $K$ w $L$.
        \item $K$ jest \deff{relatywnie algebraicznie domknięte} w $L$ $\iff$ $K_{alg}(L)=K$.
    \end{enumerate}
\end{definicja}
