...
\begin{remark}[algebraiczne rozszerzenia ciał]
    %\begin{enumerate}
        %\item 
...
\begin{remark}[lemat Gaussa: $F_m$ nierozkładalny w $\Q$]
    [\emph{Lemat Gaussa}] $F_m(x)$ jest wielomianem nierozkładalnym w $\Q[X]$ (równoważnie w $\Z[X]$).
\end{remark}
...
\begin{remark}[pierwiastek pierwotny a $dim_\Q(\Q(b))$]
...
\begin{lemma}[lemat Liouville'a o aproksymacji diofatycznej] \emph{[lemat Liouville'a o aproksymacji diofantycznej]}: Jeżeli $a\in\R$ jest liczbą algebraiczną stopnia $N>1$, to istnieje $c=c(a)\in\R_+$ takie, że dla każdego $r=\frac pq\in\Q$ zachodzi
...
\begin{definition}[algebraiczne domknięcie]
    Ciało $L\supseteq K$ jest \important{algebraicznym domknięciem} $K$ wtedy i tylko wtedy, gdy:
    \begin{enumerate}
...
\begin{remark}[istnieje algebraiczne domknięcie]
...
\begin{theorem}[jedyność domknięcia algebraicznego]\label{tw:4:15}
...
\begin{conclusion}[$K\cong L\implies\hat{K}\cong\hat{L}$]\label{wniosek:5:10}
...
\begin{remark}[algebraiczne rozszerzenie $1-1$ $\to$ $\hat{K}$]
...
%\textbf{Dowód:} Mamy dane $K\subseteq L\subseteq\hat{L}$ rozszerzenia algebraiczne, zatem rozszerzenie $K\subseteq\hat{L}$ jest algebraiczne. Stąd $\hat{L}$ jest algebraicznym domknięciem $K$. Z theoremierdzenia \ref{tw:4:15} istnieje izomorfizm $g:\hat{L}\to\hat{K}$ taki, że $g\restriction K=id_K$. Wtedy $f=g\restriction L$ jest szukanym monomorfizmem.
